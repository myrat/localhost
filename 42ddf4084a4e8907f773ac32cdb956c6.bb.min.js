...
BB.highlightCyrillic = function(t) {
 t.innerHTML = t.innerHTML.replace(/[а-яёі]+/gi, '<span class="cyrillic-char">$&</span>');
};
...
(function() {
 ...
 BB.injectMaxTopicWidthCss = function() {
  var t = $('.poster_info:first');
  if (t.length && t.is(':visible')) {
   injectCSS([ '.post_wrap { max-width: ' + Math.round($(window).width() - t.outerWidth() - 32) + 'px }' ].join('\n'));
  }
 };
})();
...
BB.banned_img_hosts_reg = /keep4u|centrkino|flashrelease|hidebehind|imagebanana|imageshack|imagevenue|imgur|interfoto|ipicture|photobucket|tinypic|(youpic|upimg|sendpic|radikal)\.ru|(rimg|ifotki)\.info/i;
...
$(function() {
 ...
 $(document).ajaxError(function(t, a) {
  var n = a.status;
  var i = a.statusText;
  if (n == 200) {
   n = '';
  ...
  e.html("Ajax error in: <i>" + ajax.url + "</i><br><b>" + n + " " + i + "</b>").show();
 ...
 var a = [ 'chrome.google.com/', 'addons.mozilla.org/' ];
 function n(t) {
 ...
 $(document).on('click mousedown touchstart', 'a.p-ext-link:not(.p-ext-link-initialized)', function t() {
  ...
  if (a.toLowerCase().startsWith('http') && !n(a)) {
   var i = {
    url: a
   };
 ...
 $('#quick-search').on('submit', function() {
  ...
  var a = $('#search-menu option:selected').data();
  var n = $.trim($('#search-text').val());
  var i = [];
  if (n == '' && e != 'search-tr') {
  ...
  case 'duckduckgo':
   BB.externalSiteSearch('duckduckgo', n);
   return false;

  case 'hash':
   if (/^[a-fA-F0-9]{40}$/.test(n)) {
    window.location.href = 'viewtopic.php?h=' + n;
   } else {
    BB.hl_quick_search_err();
   }
   return false;
 ...
 $('.ext-site-search').on('click', function() {
  ...
  var n = t.data('search_type');
  BB.externalSiteSearch(n, a);
  return false;
 });
 var o;
 var s = function(t) {
